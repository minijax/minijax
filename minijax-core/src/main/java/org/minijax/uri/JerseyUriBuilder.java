...
public class JerseyUriBuilder extends UriBuilder {
    ...
    private JerseyUriBuilder(final JerseyUriBuilder that) {
        ...
        host = that.host;
        port = that.port;
        path = new StringBuilder(that.path);
        matrixParams = that.matrixParams == null ? null : new MultivaluedStringMap(that.matrixParams);
        query = new StringBuilder(that.query);
        queryParams = that.queryParams == null ? null : new MultivaluedStringMap(that.queryParams);
    ...
    private void schemeSpecificPart(final UriParser parser) {
        ...
        if (parser.getAuthority() != null) {
            if (parser.getUserInfo() == null && parser.getHost() == null && parser.getPort() == null) {
                authority = encode(parser.getAuthority(), UriComponent.Type.AUTHORITY);
                userInfo = null;
                host = null;
                port = null;
            } else {
